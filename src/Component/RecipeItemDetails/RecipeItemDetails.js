...
const RecipeItemDetails = (props) => {
    ...
    function NextButton() {
        ...
        function handleClick() {
            if ((recipe.id+1) <= recipes.length) {
                history.push(`/recipes/${nextRec.id}`);
            }
        }
      
        return (
          <button className="btn btn-danger recipe__btn" type="button" onClick={handleClick}>
            Next Recipe 
          </button>
        );
      }
      ...
      function PrevButton() {
        ...
        return (
          <button className="btn btn-danger recipe__btn mr-1" type="button" onClick={handleClick}>
            Previous Recipe 
          </button>
        );
      }
